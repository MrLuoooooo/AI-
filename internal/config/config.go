...
func setDefaults(v *viper.Viper) {
	...
	v.SetDefault("vision.frame_interval", 1000)
	v.SetDefault("vision.frame_quality", 30)
	v.SetDefault("vision.max_frame_width", 640)
	v.SetDefault("vision.vad_mode", 1)
	v.SetDefault("vision.vad_frame_ms", 30)
	v.SetDefault("vision.vad_silence_ms", 800)
	v.SetDefault("vision.max_token_per_min", 0)
	v.SetDefault("vision.max_sessions", 10)
	v.SetDefault("vision.system_prompt", "你是一个 AI 视觉助手。请根据摄像头画面和用户的语音输入，用中文简洁回答。可以描述你看到的内容、回答用户关于画面中事物的提问。")
	v.SetDefault("vision.vision_model", "gpt-4o")
	v.SetDefault("vision.detail", "low")
}
